...
raw_refs = """1.  Fonsêca  ALA,  Barbalho  IMP,  Fernandes  F,  et  al.Blockchain in health information systems: a systematicreview.Int J Environ Res Public Health2024; 21: 1512.
...
items = re.findall(r'(\d+)\.\s+(.*?)(?=\n\d+\.\s+|\Z)', raw_refs, re.DOTALL)

html_list = []
for num, text in items:
    text = text.replace('\n', ' ').strip()
    html_list.append(f"                        <li>{text}</li>")

html_str = "\n".join(html_list)

replacement_block = f"""                <hr style="margin: 30px 0; border: 0; border-top: 1px solid var(--outline-variant);" />
                <h3>Referencias (Lista Completa)</h3>
                <div style="background: var(--surface-container-low); padding: 15px; border-radius: 8px; font-size: 0.75rem; max-height: 350px; overflow-y: auto;">
                    <ol style="padding-left: 20px; margin: 0; color: var(--on-surface-variant); line-height: 1.5; font-family: var(--font-body);">
{html_str}
                    </ol>
                </div>"""

with open("c:/Users/AlbertG/IGGA.SAS/PERSONAL/DEV/SHEY/Diana_MIsaza/app.js", "r", encoding="utf-8") as f:
    content = f.read()

# Replace the block
start_marker = "                <hr style=\"margin: 30px 0; border: 0; border-top: 1px solid var(--outline-variant);\" />"
end_marker = "                    </ol>\n                </div>"
...
if start_idx != -1 and end_idx != -1:
    ...
    with open("c:/Users/AlbertG/IGGA.SAS/PERSONAL/DEV/SHEY/Diana_MIsaza/app.js", "w", encoding="utf-8") as f:
        f.write(new_content)
    print("Success")
else:
